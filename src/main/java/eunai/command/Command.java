...
public class Command {
     ...
     * Routes to either mass operations or single operations based on the input format.
     *
     * @param input   The user input command as a string.
     * @param tasks   The {@link TaskList} that stores the current tasks.
     * @param ui      The {@link Ui} object to interact with the user (not used here, but kept for consistency).
    ...
    public static String execute(String input, TaskList tasks, Ui ui, Storage storage) {
        ...
        if (input.contains(",")) {
            // If user wants to mark/unmark/delete multiple tasks at once.
    ...
    /**
     * Routes the command to the correct handler method via a switch statement.
     */
    ...
    /**
     * Handles mass operations (mark, unmark, delete) involving multiple task indices.
     */
    ...
    private static void processMassOperation(CommandParser.Command commandType,
                                             String indexStr, TaskList tasks, StringBuilder result) {
        try {
    ...
    private static String handleMark(String input, TaskList tasks) {
        try {
            ...
            return "Sorry, please enter a valid task number after 'mark'.";
        }
    }
    ...
    private static String handleFind(String input, TaskList tasks) {
        try {
            ...
            if (foundTasks.getSize() == 0) {
                return "No tasks match that search. Try again!";
            }
            ...
            return "Invalid search. Please provide a keyword after 'find'.";
        }
    }
    ...
    private static String handleExit(TaskList tasks, Storage storage) {
        ...
        if (!isSaved) {
            return "Oops! I couldn't save your tasks. Something went wrong.";
        }
        ...
        return "Byeee! I've saved your tasks! See you next time!";
    }
